...
Note: the build script is written in **TypeScript** and uses `tsx`. Running `npm install` will install `tsx` (dev dependency) so the script can run. The script will output HTML files into an `articles/` folder and generate an `articles/index.html` listing (accessible as `/articles/`).
