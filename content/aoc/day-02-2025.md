...
The approach is straightforward — the check is done inline:
...
JavaScript's `String.prototype.repeat()` does the heavy lifting here. We extract the first `len` characters, repeat them enough times to match the original length, and compare.
