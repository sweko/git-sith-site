...
const CONTENT_DIR = path.join(__dirname, '..', 'content', 'student-projects');
const OUTPUT_DIR = path.join(__dirname, '..', 'student-projects');
const SITE_TITLE = 'GitSith - Student Projects';
...
function build() {
  ...
  if (fs.existsSync(OUTPUT_DIR) && path.basename(OUTPUT_DIR) === 'student-projects') {
  ...
  if (!fs.existsSync(CONTENT_DIR)) {
    console.log(`No content directory found at ${CONTENT_DIR} — nothing to build`);
    return;
  }

  const mdFiles: string[] = fs.readdirSync(CONTENT_DIR).filter((f: string) => f.endsWith('.md'));

  const projects: ProjectRecord[] = mdFiles.map((file: string) => {
    ...
    // Remove leading H1 from markdown content if it matches the project title (prevents duplicated H1)
...
<head>
  ...
  <link rel="canonical" href="${BASE_URL}/student-projects/${slug}/">
...
<body>
  ...
  <main>
    <div class="article-container container">
      <header class="article-header">
        ...
        <div class="article-meta">${readingMinutes} min read</div>
      ...
      <footer class="article-footer">
        <p>Student project hosted on GitSith.</p>
      </footer>
    </div>
  </main>
</body>
    ...
    return { title, description, slug, filename: `${slug}.html`, srcFile: file } as ProjectRecord;
  });

  // Student projects are only exposed via direct links; do not generate an index page
  ...
  console.log(`Built ${projects.length} student projects to ${OUTPUT_DIR}`);
}
